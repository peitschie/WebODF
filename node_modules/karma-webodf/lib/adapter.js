...
(function (win) {
    ...
    function createStartFn(karma) {
        return function () {
            var testCount = 0;
            tester.resourcePrefix = "resource/";
            tester.reporter = function (r) {
                r.log = r.log.filter(function(stuff) { return stuff.category !== "pass"; });
                r.log.forEach(function (stuff, i) {
                    r.log[i] = stuff.category + ":" + stuff.message;
                });
                testCount += 1;
    ...
    win.WEBODF_CURRENT_DIR = "/base/lib";
    win.WEBODF_LIB_PATHS = ["/base/lib", "/base/tests"];
}(window));
